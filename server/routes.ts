...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/docs/javascript-installation-guide', isAuthenticated, async (req, res) => {
    const guide = `# JavaScript/TypeScript SDK - Installation Guide
...
\`\`\`javascript
const { AveroxCrypto, configureTelemetry } = require('@averox/crypto-sdk');
...
### TypeScript Setup
\`\`\`typescript
import { AveroxCrypto, AveroxEnvelope, configureTelemetry } from '@averox/crypto-sdk';
...
\`\`\`bash
# Verify installation
npm list @averox/crypto-sdk

# Reinstall if necessary
npm uninstall @averox/crypto-sdk
npm install @averox/crypto-sdk
\`\`\`

#### 2. TypeScript Import Issues
Check your tsconfig.json configuration for proper module resolution.
...
`;
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="javascript-installation-guide.md"');
    res.send(guide);
  });

  // Python Installation Guide
  app.get('/api/docs/python-installation-guide', isAuthenticated, async (req, res) => {
    const guide = `# Python SDK - Installation Guide
...
\`\`\`bash
# Check installation
pip show averox-crypto

# Reinstall if needed
pip uninstall averox-crypto
pip install --no-cache-dir averox-crypto
\`\`\`
...
`;
    ...
    res.setHeader('Content-Type', 'text/markdown');
    res.setHeader('Content-Disposition', 'attachment; filename="python-installation-guide.md"');
    res.send(guide);
  });

  // Universal Troubleshooting Guide
  app.get('/api/docs/universal-troubleshooting-guide', isAuthenticated, async (req, res) => {
    const guide = `# Universal SDK Troubleshooting Guide
...
`;
    ...
    res.setHeader('Content-Type', 'text/markdown');
    res.setHeader('Content-Disposition', 'attachment; filename="universal-troubleshooting-guide.md"');
    res.send(guide);
  });

  // Encryption Failure Debugging Guide
  app.get('/api/docs/encryption-failure-debugging', isAuthenticated, async (req, res) => {
    const guide = `# Encryption Failure Debugging Guide
...
`;
    ...
    res.setHeader('Content-Type', 'text/markdown');
    res.setHeader('Content-Disposition', 'attachment; filename="encryption-failure-debugging.md"');
    res.send(guide);
  });
