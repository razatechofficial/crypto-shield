...
export const rotationTriggerEnum = pgEnum("rotation_trigger", [
  "time_based",
  "usage_based",
  "manual",
  "compromise_detected",
...
export const keyVersionStatusEnum = pgEnum("key_version_status", [
  "current",
  "previous",
  "deprecated",
  "compromised",
...
// SDK language enum
export const sdkLanguageEnum = pgEnum("sdk_language", [
  "javascript",
  "python",
  "java",
  "csharp",
...
export const confidentialFeatureEnum = pgEnum("confidential_feature", [
  "teeEncryption",
...
// HSM provider enum
export const hsmProviderEnum = pgEnum("hsm_provider", [
  "safenet",
  "thales",
  "ncipher",
  "aws_cloudhsm",
...
export const hsmConnectionTypeEnum = pgEnum("hsm_connection_type", [
  "pkcs11",
  "kmip",
  "rest_api",
  "proprietary",
...
export const hsmDeviceStatusEnum = pgEnum("hsm_device_status", [
  "online",
  "offline",
  "maintenance",
  "error",
...
export const smartTokenTypeEnum = pgEnum("smart_token_type", [
  "yubikey_piv",
  "yubikey_fido2",
  "smartcard_piv",
  "smartcard_cac",
  "pkcs15_token",
  "tpm2_token",
  "mobile_secure_element",
]);

// HSM session status enum
export const hsmSessionStatusEnum = pgEnum("hsm_session_status", [
  "active",
  "idle",
  "expired",
  "terminated",
...
export const fipsValidationLevelEnum = pgEnum("fips_validation_level", [
  "level_1",
  "level_2",
  "level_3",
  "level_4",
...
export const keyUsagePolicyEnum = pgEnum("key_usage_policy", [
  "unrestricted",
  "sign_only",
  "encrypt_only",
  "time_limited",
...
export const hsmAuditEventTypeEnum = pgEnum("hsm_audit_event_type", [
  "login",
  "logout",
  "key_access",
  "key_modification",
...
export const users = pgTable("users", {
  ...
  subscriptionPlan:
    subscriptionTierEnum("subscription_plan").default("starter"),
...
export const sdks = pgTable("sdks", {
  ...
  vaultKekAlgorithm: varchar("vault_kek_algorithm"), // aes256-gcm96, chacha20-poly1305
  vaultKekVersion: integer("vault_kek_version"), // Current KEK version
  envelopeEncryptionEnabled: boolean("envelope_encryption_enabled").default(
    true
  ),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
...
export const encryptionKeys = pgTable(
  ...
  (table) => {
    return {
      ...
      tenantKeyUnique: uniqueIndex("unique_tenant_key").on(
        table.tenantId,
        table.id
      ),
      ...
      rotationScheduleIdx: index("idx_keys_rotation_schedule").on(
        table.tenantId,
        table.nextRotationAt,
        table.status
      ),

      // Index for version queries
      versioningIdx: index("idx_keys_versioning").on(
        table.parentKeyId,
        table.version,
        table.versionStatus
      ),
      ...
      usageTrackingIdx: index("idx_keys_usage_tracking").on(
        table.tenantId,
        table.usageCount,
        table.maxUsageCount
      ),
...
export const keyRotationPolicies = pgTable("key_rotation_policies", {
  ...
  algorithmId: varchar("algorithm_id").references(
    () => encryptionAlgorithms.id
  ),
...
export const keyRotationHistory = pgTable(
...
export const hsmKeys = pgTable("hsm_keys", {
  ...
  keyUsagePolicy:
    keyUsagePolicyEnum("key_usage_policy").default("unrestricted"),
  status: keyStatusEnum("status").default("active"),
...
export const encryptionKeyRelations = relations(
  encryptionKeys,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const keyRotationPolicyRelations = relations(
  keyRotationPolicies,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const keyRotationHistoryRelations = relations(
  keyRotationHistory,
  ({ one }) => ({
    tenant: one(tenants, {
...
export const insertSdkSchema = createInsertSchema(sdks)
  .omit({
    id: true,
    createdAt: true,
    updatedAt: true,
...
export const insertEncryptionKeySchema = createInsertSchema(
  encryptionKeys
).omit({
  id: true,
  createdAt: true,
...
export const insertKeyRotationPolicySchema = createInsertSchema(
  keyRotationPolicies
).omit({
  id: true,
  createdAt: true,
...
export const insertKeyRotationHistorySchema = createInsertSchema(
  keyRotationHistory
).omit({
  id: true,
  createdAt: true,
...
export type InsertKeyRotationPolicy = z.infer<
  typeof insertKeyRotationPolicySchema
>;
export type KeyRotationHistory = typeof keyRotationHistory.$inferSelect;
export type InsertKeyRotationHistory = z.infer<
  typeof insertKeyRotationHistorySchema
>;
...
export const insertCryptoOperationSchema = createInsertSchema(
  cryptoOperations
).omit({
  id: true,
  createdAt: true,
...
export const insertSdkDeploymentSchema = createInsertSchema(
  sdkDeployments
).omit({
  id: true,
  createdAt: true,
...
export const insertComplianceAssessmentSchema = createInsertSchema(
...
export const insertCertificateAuthoritySchema = createInsertSchema(
...
export type InsertComplianceAssessment = z.infer<
  typeof insertComplianceAssessmentSchema
>;
export type CertificateAuthority = typeof certificateAuthorities.$inferSelect;
export type InsertCertificateAuthority = z.infer<
  typeof insertCertificateAuthoritySchema
>;

// Relations for new tables
export const cryptoOperationRelations = relations(
  cryptoOperations,
  ({ one }) => ({
    tenant: one(tenants, {
...
export const performanceMetricRelations = relations(
  performanceMetrics,
  ({ one }) => ({
    tenant: one(tenants, {
...
// HSM Relations
export const hsmProviderRelations = relations(
  hsmProviders,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const complianceAssessmentRelations = relations(
  complianceAssessments,
  ({ one }) => ({
    tenant: one(tenants, {
...
export const certificateAuthorityRelations = relations(
  certificateAuthorities,
  ({ one }) => ({
    tenant: one(tenants, {
...
export const byokImportStatusEnum = pgEnum("byok_import_status", [
  "pending_validation",
  "imported",
  "validation_failed",
  "expired",
...
export const keyDistributions = pgTable(
  "key_distributions",
  {
    id: varchar("id")
      .primaryKey()
    ...
    keyId: varchar("key_id")
      .references(() => encryptionKeys.id)
      .notNull(),
    providerConfigId: varchar("provider_config_id")
    ...
    distributionStatus: distributionStatusEnum("distribution_status").default(
      "pending"
    ),
  ...
  (table) => [
    uniqueIndex("unique_key_provider_distribution").on(
      table.keyId,
...
export const keyReplications = pgTable("key_replications", {
  ...
  tenantId: varchar("tenant_id")
    .references(() => tenants.id)
    .notNull(),
  sourceDistributionId: varchar("source_distribution_id")
    .references(() => keyDistributions.id)
    .notNull(),
  targetProviderConfigId: varchar("target_provider_config_id")
    .references(() => cloudProviderConfigs.id)
    .notNull(),

  // Replication settings
  replicationStatus:
    replicationStatusEnum("replication_status").default("in_progress"),
...
export const byokImports = pgTable("byok_imports", {
  ...
  // Import details
  importStatus:
    byokImportStatusEnum("import_status").default("pending_validation"),
...
export const cloudProviderConfigRelations = relations(
...
export const keyDistributionRelations = relations(
  keyDistributions,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const keyReplicationRelations = relations(
  keyReplications,
  ({ one }) => ({
    tenant: one(tenants, {
...
export const compliancePolicyRelations = relations(
  compliancePolicies,
  ({ one }) => ({
    tenant: one(tenants, {
...
export type InsertCloudProviderConfig =
  typeof cloudProviderConfigs.$inferInsert;
...
export const insertCloudProviderConfigSchema = createInsertSchema(
  cloudProviderConfigs
).omit({
  id: true,
  createdAt: true,
...
export const insertKeyDistributionSchema = createInsertSchema(
  keyDistributions
).omit({
  id: true,
  createdAt: true,
...
export const insertKeyReplicationSchema = createInsertSchema(
  keyReplications
).omit({
  id: true,
  createdAt: true,
...
export const insertCompliancePolicySchema = createInsertSchema(
  compliancePolicies
).omit({
  id: true,
  createdAt: true,
...
export const extendedUserRoleEnum = pgEnum("extended_user_role", [
  "admin",
  "manager",
  "developer",
  "viewer",
...
export const tenantUsers = pgTable(
  ...
  (table) => [
    uniqueIndex("unique_user_tenant_membership").on(
      table.tenantId,
      table.userId
    ),
  ]
...
export const subscriptionPlans = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("unique_stripe_product")
      .on(table.stripeProductId)
      .where(sql`stripe_product_id IS NOT NULL`),
    uniqueIndex("unique_stripe_price")
      .on(table.stripePriceId)
      .where(sql`stripe_price_id IS NOT NULL`),
    uniqueIndex("unique_paypal_product")
      .on(table.paypalProductId)
      .where(sql`paypal_product_id IS NOT NULL`),
    uniqueIndex("unique_paypal_plan")
      .on(table.paypalPlanId)
      .where(sql`paypal_plan_id IS NOT NULL`),
...
export const approvals = pgTable(
  ...
  (table) => [
    uniqueIndex("unique_workflow_approver").on(
      table.workflowId,
      table.approverId
    ),
